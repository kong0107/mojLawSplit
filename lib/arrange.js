...
const contentParser = (arr, pcode, lang) => {
    ...
    const pool = arr.map(item => {
        if(item['條號']) {
            ...
            const number = parseInt(m[1]) * 100 + parseInt(m[2] ? m[3] : 0);
            const paras = [];
            const lines = item['條文內容'].split("\r\n");
            for(let temp = "", i = 0; i < lines.length; ++i) {
                const line = lines[i];
                if(/[：。]$/.test(line)) {
                    paras.push(temp + line.trimStart());
                    temp = "";
                }
                else {
                    if(temp) temp += line.trimStart();
                    else temp = line;
                }
            }
            const article = {number, content: paras};
        ...
        if(isEng) {
            ...
            if(m = text.match(/^第\s*([零一二三四五六七八九十百]+)\s*([編章節款目])(之([一二三四五六七八九十百]+))?/)) {
                type = m[2];
    ...
    const result = {divisions: nested, articles};

    /// 找母法，只依照句型去推測名稱，不會確認該名稱的法律是否真的存在，遇到雙母法的情形會出錯。
    // let match;
    // if(!isEng && (match = articles[0].content.match(/依[據照]?「?([^」]+)」?\s*[(（﹙]以下簡稱本(法|律|條例|通則)[﹚）)]/))) {
    //     let title = match[1].replace(/\s+/g, ''); // C0010021
    //     title = title.replace(/(第[一二三四五六七八九十]+[條項款目])+/, ''); // A0020182, H0070052
    //     return Object.assign({theAct: {title}}, result);
    // }
...
const historyParser = (text, pcode, lang) => {
    ...
    if(lang === 'english') {
        ...
        console.assert(/^\d+\./.test(text), pcode); /// 有些直接從 "2." 開始，有些是有個空白的 "1."
        const paras = text.substring(2).split(/\d+\./);
        history = paras.map(para => {
            para = para.trim();
 ...
 * 處理「生效內容」，有些有時會有分項。
 */
const effectiveContentParser = text => {
    let paras;
    if(!text.startsWith("1.")) paras = [text];
    else paras = text.substring(2).split(/\r?\n\d+\./);
    const effectiveContent = paras.map(para =>
        para.trim()
        .replaceAll(/\r?\n\s*/g, '')
        .replaceAll(/\s+/g, ' ')
    ).join("\n");
 ...
 * 處理「附件」。
 */
const attachmentsParser = arr => ({
    attachments:
        arr.map(atta => {
            const id = atta['下載網址'].substring(55);
            return {name: atta['檔案名稱'], id};
        })
});
...
const reassigner = {
    ...
    '法規名稱': 'title',
    '法規網址': 'url',
    '法規類別': (list => ({categories: list.split('＞')})),
    '最新異動日期': 'lastUpdate',
    '生效日期': 'effectiveDate',
    '生效內容': effectiveContentParser,
    '廢止註記': (val => ({discarded: !!val})),
    '是否英譯註記': (val => ({translated: val !== 'N'})),
    '英文法規名稱': 'english',
    '附件': attachmentsParser,//(arr => arr.map(atta => ({name: atta['檔案名稱'], id: })), //'attachments',
    '沿革內容': historyParser,
